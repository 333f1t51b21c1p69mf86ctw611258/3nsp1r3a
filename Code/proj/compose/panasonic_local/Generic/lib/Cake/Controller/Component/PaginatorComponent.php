 ...
 */
	public function paginate($object = null, $scope = array(), $whitelist = array()) {
        ...
        #$this->log('=== after translate ===', 'debug');
        #$this->log($object, 'debug');
        #$this->log('=== after translate end ===', 'debug');
        ...
        // Briode specific
        if ( !empty($object->assocModelNames ) ){
            $extra['assocModelNames'] = $object->assocModelNames;
        }
	...
	public function validateSort(Model $object, array $options, array $whitelist = array()) {
		...
		if (!empty($options['order']) && is_array($options['order'])) {
			...
			foreach ($options['order'] as $key => $value) {
                ...
                // BRIODE: allow space ending field
				if ($object->hasField(trim($field))) {
