 ...
 */
	public function generateAssociationQuery(Model $model, $linkModel, $type, $association, $assocData, &$queryData, $external, &$resultSet) {
        ...
        // Briode
        // Paginator only runs this code
        $assocModelNames = array();
        $group_by = $queryData['group'];
        if( isset($queryData['assocModelNames']) ){
            $assocModelNames = $queryData['assocModelNames'];
            if( isset($queryData['group']) ){
                // FIXME this may not be correct
                $group_by += $modelAlias.'.id';
            }else{
                $group_by = array($modelAlias.'.id');
            }
        }
		...
		if ($linkModel === null) {
			return $this->buildStatement(
				array(
					...
					'group' => $group_by,
                    'assocModelNames' => $assocModelNames
				),
				$model
			);
		}
...
/**
 * Generates <schema>.<tableName> as <modelName> representation for adding model listing in SELECT FROM table listings.
 *
 * @param array $query A data structure for generating SQL query
 * @return string An SQL table alias listing all ssociated models
 */
    private function genAssocModelNameList($query){
        $ret = ' ';
        if( !isset($query['assocModelNames']) ) return $ret;

        $schema_name = $this->getSchemaName();
        foreach( $query['assocModelNames'] as $modelName ){
            $ret .= ', ';
            $ret .= '`'.$schema_name.'`.`'.strtolower(Inflector::tableize($modelName)) .'` AS `'.$modelName.'`';
        }
        $ret .= ' ';
 ...
 */
	public function buildStatement($query, $model) {
		...
		return $this->renderStatement('select', array(
            ...
            'assocModelNames' => $this->genAssocModelNameList($query)
		));
	}
 ...
 */
	public function renderStatement($type, $data) {
		...
		switch (strtolower($type)) {
			case 'select':
				#$this->log( "SELECT {$fields} FROM {$table} {$alias} {$assocModelNames} {$joins} {$conditions} {$group} {$order} {$limit}", 'debug');
				return "SELECT {$fields} FROM {$table} {$alias} {$assocModelNames} {$joins} {$conditions} {$group} {$order} {$limit}";
			case 'create':
